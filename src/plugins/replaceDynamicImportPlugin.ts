  ...
  setup: (build) => {
    build.onResolve({ filter: /.*/ }, async (args) => {
      const { dir, name } = parsePath(
        resolvePathOrReturnOrginal(args.resolveDir, args.path),
      );
      ...
      if (
        args.importer.endsWith('node_modules/ipfs-utils/src/http.js') &&
        args.path === './http/fetch'
      ) {
        console.log(
          '>',
          JSON.stringify(args),
      ...
      if (
        ...
        console.log(
          'Replacing for browser:',
          JSON.stringify(args),
          browserCandidatePath,
          '\n\n',
        );
    ...
    build.onLoad(
      { filter: /fr32-sha2-256-trunc254-padded-binary-tree-multihash/ },
      async (_args) => {
